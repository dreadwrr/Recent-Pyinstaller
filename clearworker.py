...
class ClearWorker(Worker):
    ...
    def __init__(self, database, target, nc, email, usr, flth, dcr):
        ...
        self.cachermPATTERNS = None  # set_cache
        self.filterhitRESET = None
                        ...
                        if action == "cache":
                            ...
                            if filter_literal is not None and cache_patterns is not None:
                                ...
                                if filter_literal is None:
                                    missing.append("filterhitRESET")
                                if cache_patterns is None:
                                    missing.append("cachermPATTERNS")
                                errmsg = f"Missing attribute(s): {', '.join(missing)}"
