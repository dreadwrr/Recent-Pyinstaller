...
# add " " to the paths for winrar 7zip file list for archive
def encase_line(recent_files, arch_exclude, filters, USR, MODULENAME):
    ...
    n_line = filter_lines_from_list(n_line, escaped_user, filters)
...
def comp_archive(recent_files, archive, arch_exclude, filters, USR, MODULENAME, zipPROGRAM, zipPATH, downloads):
    ...
    encase_line(recent_files, arch_exclude, filters, USR, MODULENAME)
...
def main(action, filename, extension, basedir, USR, dspEDITOR, dspPATH, tempdir, cutoffTIME=None, zipPROGRAM=None, zipPATH=None, USRDIR=None, downloads=None):
    ...
    try:
        if action == "wsl":
            ...
            try:
                for item in os.listdir(basedir):
                    ...
                    if os.path.isdir(b_path):
                        base_folders.append(b_path)
            except (OSError, PermissionError):
                is_progress = False

            base_folder_paths = tuple(base_folders)
            ...
            args.extend(['-mergedRs', recent_files, '-excluded', excl_path])  # .ps1 resolves path from tgt_file (recent_files) or excl_file
        ...
        if res == 0 and os.path.isfile(recent_files) and os.path.getsize(recent_files) != 0:
            ...
            if os.path.isfile(target_f) and cutoffTIME is not None:
                ...
                flth_frm = localappdata / "flth.csv"
                dbtarget_frm = localappdata / "recent.gpg"
                ...
                CACHE_S, _ = os.path.splitext(CACHE_S)  # to match index drives as well as CACHE_S

                arch_exclude = get_runtime_exclude_list(dbtarget, localappdata, tempdir, MODULENAME, USRDIR, flth, CACHE_F, CACHE_S)  # exld from archive .rar/.zip  inclusions this app and temp work area

                filter_path = localappdata / "filter.toml"
                filter_list = load_config(filter_path)
                ...
                res = comp_archive(target_f, archive, arch_exclude, filters, USR, MODULENAME, zipPROGRAM, zipPATH, downloads)
