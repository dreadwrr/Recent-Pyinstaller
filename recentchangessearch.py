...
def main(argone, argtwo, USR, PWD, argf="bnk", method="", iqt=False, db_output=None, POST_OP=False, scan_idx=False, showDiff=False, argwsl=False, dspPATH=None):
    ...
    flth_frm = appdata_local / "flth.csv"  # res_path(config['paths']['flth'], USR)  # filter hits
    dbtarget_frm = appdata_local / "recent.gpg"  # res_path(config['paths']['pydbpst'], USR)
    CACHE_F_frm = appdata_local / "ctimecache.gpg"  # res_path(config['paths']['CACHE_F'], USR)
    ...
    with tempfile.TemporaryDirectory(dir=TEMPD) as mainl:
        ...
        if THETIME != "noarguser":
            ...
            try:
                ...
                if not parseflnm:  # get directory name
                    # parseflnm = filename.rstrip('/').split('/')[-1] #linux directory
        ...
        # the start time is accurately stored before merging ctime results
        ...
        TMPOPT = filter_lines_from_list(TMPOPT, escaped_user, filter_escaped)
        ...
        if SORTCOMPLETE:
            ...
            if ANALYTICSECT:
                ...
                print()
            # Diff output to user
            csum = processha.processha(rout, ABSENT, diffnm, cerr, flsrh, argf, SRTTIME, escaped_user, supbrw, supress, supbrwLIST, filter_escaped)

            # Filter hits
            update_filter_csv(RECENT, flth, escaped_user, filter_escaped)
            sys.stdout.flush()

            # Terminal output process scr/cer
            if not csum and not supress:
                if os.path.exists(slog):  # escaped_user
                    filter_output(slog, 'Checksum', 'no', 'blue', 'yellow', 'scr', supbrwLIST, supbrw, supress)
        ...
        if dbopt not in ("new_profile", "encr_error") and scanIDX:  # Scan system index. If it is from the command line and a new profile was just made dont scan it. Encryption failure dont scan as there is a problem.
            ...
            # append to old or use new defaul
            if diffrlt:
                diff_file = diffnm
            else:
            ...
            # from commandline, turn off so doesnt scan every time
            if not iqt:
            ...
            if rlt != 0:
                if rlt == 1:
                    ...
                    return 1
                if rlt == 7:  # commandline no profile table or data yet
                    if not iqt:
