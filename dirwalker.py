#   build first to find the files then distribute round-robin to multiprocessing            12/09/2025
...
def find_created(dbopt, dbtarget, basedir, mdltype, tempdir, CACHE_S, dspEDITOR, dspPATH, email, ANALYTICSECT=True, compLVL=200):
    ...
    if mdltype.lower() == "hdd":
        ...
        start = time.time()
        with ProcessPoolExecutor(max_workers=total_chunks) as executor:  # min(total_chunks, max_workers)
        ...
        if systime_results:
            ...
            try:
                ...
                if cfr_data:  # update the cache with delta
                    ...
                    # flatten dict of dict for database upsert
                    key_upt = []
                    for folder, data in cfr_data.items():
                        key_upt.append((
                            none_if_empty(data['modified_time']),
                            folder,
                            data.get('file_count'),
                            data.get('max_depth')
                        ))
...
def index_system(dbopt, dbtarget, basedir, updatehlinks, CACHE_S, email, ANALYTICSECT=False, idx_drive=False, cache_idx=True, compLVL=200, iqt=False, strt=0, endp=100):
    ...
    def create_new_index():
        ...
        if dir_data:
            ...
            return index_drive(dbopt, dbtarget, basedir, None, parsedidx, dir_data, CACHE_S, "Reencryption failed drive idxcache not saved.", email, idx_drive=idx_drive, compLVL=compLVL, iqt=iqt, dcr=False)
    ...
    if rlt == 0:
        ...
        if parsedsys:
            ...
            rlt = index_drive(dbopt, dbtarget, basedir, parsedsys, parsedidx, dir_data, CACHE_S, "Reencryption failed sys idxcache not saved.", email, idx_drive=False, compLVL=compLVL, iqt=iqt, dcr=False)  # save cache file and store in db
            if rlt == 0:
                print("Progress: 100%", flush=True)
...
def scan_system(dbopt, dbtarget, basedir, difffile, updatehlinks, CACHE_S, email, ANALYTICSECT=True, showDiff=False, cache_idx=False, compLVL=200, dcr=False, iqt=False, strt=0, endp=100):
    ...
    if rlt == 0:
        ...
        if all_sys:
            ...
            if save_db(dbopt, dbtarget, basedir, CACHE_S, email, None, None, all_sys, keys=None, idx_drive=False, compLVL=compLVL, iqt=iqt, dcr=dcr):
