...
def hanly(parsed_chunk, cachermPATTERNS, checksum, cdiag, dbopt, ps, ll_level, sys_tables, chunk_index, special_k, strt=65, endp=90):
    ...
    with sqlite3.connect(dbopt) as conn:
        ...
        for record in parsed_chunk:
            ...
            if not previous or len(previous) < 11:
            ...
            if (is_integer(record[3]) and is_integer(previous[3])  # format check
                ...
                if "." in recent_mod_time and int(recent_mod_time.split(".")[1]) == 0:
                    entry["scr"].append(f'Unusual modified time has microsecond all zero: {label} timestamp: {recent_mod_time}')
