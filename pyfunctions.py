...
def sbwr(webb):  # note regex

    compiled = [re.compile(p.replace("\\", "\\\\")) for p in webb]

    return compiled


# Database cache clear
def get_delete_patterns(usr, cache_patterns):

    return [p.replace("{user}", usr) for p in cache_patterns]  # qt app


# for resetting filterhits CSV on cache clear add the literal here. copy from filter
...
def goahead(filepath):
    try:
        ...
        logging.debug(f"Skipping exception stating file {filepath.name}: {type(e).__name__} - {e}", exc_info=True)
    return None
...
def get_wdir():
    ...
    if getattr(sys, "frozen", False):
        ...
        base_path = Path(__file__).resolve().parent  # base_path = Path(sys.argv[0]).resolve().parent
    wdir = base_path
