# 12/06/2025               Qt gui windows 11                  Developer buddy 3.0
import glob
import logging
...
from clearworker import ClearWorker
from dbworkerstream import DbWorkerIncremental
from imageraster import raised_image
from mftworker import MftWorker
from processhandler import ProcessHandler
from pstsrg import create_db
from pyfunctions import get_wdir
from pyfunctions import is_integer
from pyfunctions import load_config
from pyfunctions import setup_logger
from qtfunctions import add_extension
from qtfunctions import check_for_updates
from qtfunctions import current_drive_type_model_check
from qtfunctions import show_cmddoc
from qtfunctions import DBMexec
from qtfunctions import DBConnectionError
from qtfunctions import DriveSelectorDialog
from qtfunctions import FastColorText
from qtfunctions import fill_extensions
from qtfunctions import get_conn
from qtfunctions import get_help
from qtfunctions import has_sys_data
from qtfunctions import help_about
from qtfunctions import load_cmdpmpt
from qtfunctions import load_explorer
from qtfunctions import load_gpg
from qtfunctions import load_pshell
from qtfunctions import open_html_resource
from qtfunctions import QTextEditLogger
from qtfunctions import save_note
from qtfunctions import user_data_from_database
from qtfunctions import user_data_to_database
from qtfunctions import set_j_settings
from qtfunctions import get_j_settings
from qtfunctions import setup_drive_settings
from qtfunctions import wdisplay_prompt
from qtfunctions import wdisplay_message
from qtfunctions import wdisplay_get_input
from qtparser import dispatch_internal
from query import clear_extn_tbl
from query import dbtable_has_data
from rntchangesfunctions import check_for_gpg
from rntchangesfunctions import check_utility
...
from rntchangesfunctions import get_cache_s
...
from rntchangesfunctions import is_admin
from rntchangesfunctions import is_wsl
from rntchangesfunctions import check_installed_app
...
from rntchangesfunctions import mftec_version
...
from wmipy import get_disk_and_volume_for_drive
from wmipy import get_mounted_partitions
from wmipy import validmft
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, appdata_local, config, toml_file, json_file, driveTYPE, dbopt, dbtarget, gpg_path, gnupg_home, dspEDITOR, dspPATH, popPATH, email, usr, cachermPATTERNS, filterhitRESET, tempdir):
        super().__init__()
        ...
        self.flth = str(flth_frm)  # res_path(config['paths']['flth'], usr)
        ...
        self.myapp = self.lclhome / "main.exe"
        self.exe_path = self.lclhome / "bin"  # /usr/local/save-changesnew/ linux
    ...
    def init_events(self):
        ...
        self.ui.actionClearh.triggered.connect(lambda: self.ui.hudt.clear())
    ...
    def x_action(self):
        ...
        if self.isexec:
            ...
            if hasattr(self, 'ui') and hasattr(self.ui, 'hudt'):
                self.ui.hudt.appendPlainText("Process has been canceled.")
        else:
    ...
    def load_user_data(self):
        ...
        user_data_from_database(self.ui.hudt, self.ui.textEdit, self.ui.combffile, self.extensions, self.dbopt)
    ...
    def load_jpg(self):
        ...
        if res == "jpg":
            ...
            crest = self.open_file_dialog(self.crestdir)
            if crest:
    ...
    def load_saved_indexes(self):
        ...
        for path in filepath:
            ...
            is_drive_c, _ = os.path.splitext(fname)  # case where basedir is not C:\\ and C:\\ is indexed
            if is_drive_c == systimename:
                if current_drive != "C:\\":
                    dindex.append("C:\\")
                continue

            part_frm = fname.split(systimename + "_", 1)[-1]  # anything after _ (drive letter for all other indexes)
    ...
    def clear_extensions(self):
        ...
        if clear_extn_tbl(self.dbopt, False):
            if encr(self.dbopt, self.dbtarget, self.email, nc, False):
    ...
    def search(self, output, THETIME, argf):
        ...
        if self.wsl:
            is_search = False
        self.proc.start_pyprocess(str(self.myapp), args, is_search=is_search, is_postop=postop, is_scanIDX=scanidx)  # self.myapp
    ...
    def ffile(self, compress, time_range=None):
        ...
        self.proc.start_pyprocess(str(self.myapp), ['ffsearch.py', action, fpath, extension, self.basedir, self.usr, self.dspEDITOR, self.dspPATH, self.tempdir])
    ...
    def mftsearch(self):
        ...
        if self.ui.mftchka.isChecked():
            if not self.ramdisk:
                if not (drive := self.get_ramdrive(drive, ["Unable to locate ramdisk", "continue?", "Yes", "No"])):
                    self.isexec = False
                    return
            else:
    ...
    def scan_idx(self):
        ...
        self.proc.start_pyprocess(str(self.myapp), ['dirwalker.py', 'scan', self.dbopt, self.dbtarget, basedir, diff_file, str(self.updatehlinks), self.CACHE_S, email, str(self.ANALYTICSECT), str(showDiff), str(self.cacheidx), str(self.compLVL), 'False', 'True'], dbopt=self.dbopt, status_message="Index scan")
    ...
    def idx_drive(self):
        ...
        if dbtable_has_data(self.dbopt, sys_tables[0]):
            self.ui.hudt.appendPlainText(f"Drive {drive} has sys profile. clear IDX and rebuild on page2")
            return

        DRIVETYPE = setup_drive_settings(drive, None, self.sj, idx_drive=True)
    ...
    def find_downloads(self, basedir="C:\\"):
        ...
        self.proc.start_pyprocess(str(self.myapp), ['dirwalker.py', 'downloads', self.dbopt, self.dbtarget, basedir, drive_type, self.tempdir, CACHE_S, self.dspEDITOR, self.dspPATH, self.email, str(self.ANALYTICSECT), str(self.compLVL)])
    #
    # End Main db task
    #
    ''' Database '''
    ...
    def reload_database(self, code, is_remove=False, tables=('logs',)):
        if code == 0:
            ...
            # loadgpg(self.dbopt, self.dbtarget, self.ui.dbmainlabel) # to roll back on failure
    ...
    def manage_sj(self, code, drive_idx, locale):
        if code == 0:
            ...
            else:  # add or remove proteus EXTN from json for basedir
                if locale == 'add':
                    extn = self.psEXTN
...
def start_main_window():
    ...
    # pyinstaller filters
    # for pyinstaller filter.py and pyfunctions exclude lists are in / "filter.toml" and / "config" / "config.toml".
    #
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        try:
            ...
            exit_code = 0
            window = MainWindow(appdata_local, config, toml_file, json_file, driveTYPE, dbopt, dbtarget, gpg_path, gnupg_home, dspEDITOR, dspPATH, popPATH, email, usr, cachermPATTERNS, filterhitRESET, tempdir)
