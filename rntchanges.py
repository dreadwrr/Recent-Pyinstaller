...
def main(argv):
    ...
    PWD = os.getcwd()
    SRCDIR = "noarguser"
    appdata_local = get_wdir()  # wdir / "save-changesnew" original design was C:\\Users\\{user}\\AppData\\Local\\save-changesnew
    ...
    flth_frm = appdata_local / "flth.csv"  # config['paths']['flth']
    dbtarget_frm = appdata_local / "recent.gpg"  # config['paths']['pydbpst']
    ...
    if argone == "search":  # `recentchanges search`
        ...
        flth = str(flth_frm)  # res_path(flth, USR)
        dbtarget = str(dbtarget_frm)  # res_path(dbtarget, USR)
