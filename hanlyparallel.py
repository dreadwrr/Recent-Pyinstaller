...
def hanly_parallel(model_type, rout, scr, cerr, parsed, cachermPATTERNS, checksum, cdiag, dbopt, ps, ll_level, sys_tables, iqt=False, strt=65, endp=90):
    ...
    if len(parsed) < 40 or model_type.lower() == "hdd":
        if iqt:
            special_k = 0
        all_results, batch_incr = hanly(parsed, cachermPATTERNS, checksum, cdiag, dbopt, ps, ll_level, sys_tables, 0, special_k, strt, endp)
    else:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(
                    hanly, chunk, cachermPATTERNS, checksum, cdiag, dbopt, ps, ll_level, sys_tables, i, special_k, strt, endp
                )
