...
import fsearch as pl1
import fsearchmft as pl2
import fsearchps1 as pl3
...
# scr / cerr logic
def filter_output(filepath, filtername, critical, pricolor, seccolor, typ, supress_list, supress_browser=True, supress=False):
    webb = sbwr(supress_list)
    flg = False
...
def find_ps1(command, DRIVETYPE, RECENT, COMPLETE, mergeddb, init, checksum, updatehlinks, cfr, FEEDBACK, logging_values, end, cstart, iqt=False, strt=20, endp=60):  # FEEDBACK,  handled in .ps1 script
    ...
    def get_recent_changes(cursor, file_table):
        ...
        if file_table not in allowed_tables:
            return None
        query = f'''
    ...
    if result is None and end is None:
        sys.exit(1)
    if not result and isinstance(end, int) or not os.path.isfile(mergeddb):
...
def find_mft(DRIVETYPE, RECENT, COMPLETE, init, checksum, cfr, FEEDBACK, logging_values, end, cstart, search_time, iqt=False, strt=20, endp=60):
    ...
    csv_data = read_mftmem(exec_path, 'C:\\$MFT', compt, iqt, strt, endval)  # search
    if not csv_data:
    ...
    if not file_entries:
        print("No new files from results of search in Mft. exiting")
        sys.exit(1)
...
def read_mftmem(exec_path, mft, compt, iqt=False, strt=0.0, endp=100.00):  # mft='C:\\$MFT'
...
def filter_lines_from_list(lines, escaped_user, exclude_patterns):
...
def is_admin():
    ...
    if not res:
        ...
        try:
            ctypes.windll.shell32.ShellExecuteW(None, "runas", script, params, None, 1)  # script was sys.executable, ie  None, "runas", sys.executable, f'"{script}" {params}', None, 1
        except Exception as e:
