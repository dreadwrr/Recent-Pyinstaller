...
def os_walk_error(list_error):
    if isinstance(list_error, PermissionError):
        logging.debug(f"os.walk Permission denied: {list_error.filename}, skipping...")
    elif isinstance(list_error, OSError):
        logging.debug(f"os.walk Error accessing in a root folder: {list_error}")
    else:
        logging.debug(f"os.walk Unexpected error: {list_error}")
        raise
